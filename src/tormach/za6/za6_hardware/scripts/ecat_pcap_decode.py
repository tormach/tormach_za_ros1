        ...
        for master in masters:
            ...
            for slave in slaves:
                ...
                for entry in entries:
                    ...
                    # sys.stderr.write(etree.tostring(entry, pretty_print=True).decode("utf-8") + "\n")
...
if __name__ == "__main__":
    ...
    d = EcatDecoder()
    d.parse_ethercat_xml(args.conf)
    if args.ethercat_pcap:
        # Read from `ethercat pcap` output
        d.ethercat_capture(d.read_packets, time=args.time)
    elif args.pcap_file:
        # Read from pcap file
        d.file_capture(args.pcap_file, d.read_packets)
    d.print_csv()
