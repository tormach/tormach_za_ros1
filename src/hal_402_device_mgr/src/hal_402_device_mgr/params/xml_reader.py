...
class EtherCATXMLReader:
    ...
    @property
    def vendor_id(self):
        ids = self.vendor_xml.xpath("Id")
        assert len(ids) == 1  # Should only ever be one
        id_str = ids[0].text
        return self.str_to_int(id_str)
    ...
    def type_data(self, o, type_idx):
        ...
        if EtherCATTypes.is_etg1020_base_type(type_name):
            self.safe_set(otype, "Type", type_name)
            self.safe_set(
                otype, "DataType", EtherCATTypes.get_etg1020_type(type_name)
            )
        return otype
        ...
        for obj in device.xpath("Profile/Dictionary/Objects/Object"):
            ...
            for i, otype in enumerate(otypes):
                ...
                if "ArrayType" in osub:
                    ...
                    type_name = osub["Type"]
                try:
                    ecat_type = EtherCATTypes.get_etg1020_type(type_name)
    ...
    def add_sdo(self, sdos, data):
        ...
        sdos[kwargs["index"], kwargs["subindex"] or 0] = sdo
    ...
    def sdos(cls, model_id):
        if model_id not in cls._device_registry:
            raise RuntimeError(
                f"Unknown model {model_id}; "
                f"known: {list(cls._device_registry.keys())}"
            )
