...
class StateMachine402:
    ...
    def voltage_enabled(self):
        ...
        return bool(
            self.status_word & (1 << self.sw_extra_bits['VOLTAGE_ENABLED'])
        )

    @property
    def can_enable(self):
        # Check for unfixable conditions that might prevent enabling drive

        # Drives can't be enabled when offline or non-operational
        if not self.slave_online:
            return (False, "Drive not online")
        if not self.slave_oper:
            return (False, "Drive not operational")

        # Drives can't go into SWITCHED ON state with no motor power
        if not self.voltage_enabled:
            return (False, "No motor power at drive")
