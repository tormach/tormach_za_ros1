...
class TestHal402MgrBase:
    ...
    patch_mock_rospy = (
        ...
        'hal_402_device_mgr.state_machine_402.rospy',
        'hal_402_device_mgr.params.params_rospy.rospy',
        'hal_402_device_mgr.params.params_base.rospy',
        'hal_402_device_mgr.params.logging.rospy',
    )
...
# FIXME state_cmd start; safety_input 0; enabling_input 0; (fault) state_cmd 2


class TestHal402MgrIS620Init(TestHal402MgrBase):
...
class TestHal402MgrSV660Init(TestHal402MgrIS620Init):
    drive_type = 'inovance_sv660n'


class TestHal402MgrIS620(TestHal402MgrBase):
    drive_type = 'inovance_is620n'
    ...
    def test_run(self, obj, mocker):
        ...
        with pytest.raises(self.ros_interrupt_exception):
            obj.run()
        assert len(mock_update.mock_calls) == 2
        obj.rate.sleep.assert_called()


class TestHal402MgrSV660(TestHal402MgrIS620):
    drive_type = 'inovance_sv660n'


class TestHal402MgrIS620Update(TestHal402MgrBase):
    ...
    @pytest.fixture
    def mock_drive_params_download(self):
        # Patch up the `download()` method to print what it would do,
        # but don't actually do anything.
        def mock_download(sdo, position, val, dry_run=False):
            print(
                f'mock_download(sdo={sdo}, position={position}, val={val}, dry_run={dry_run}'
            )

        patch(
            'hal_402_device_mgr.params.EtherCATDriveParams.download',
    ...
    @pytest.fixture
    def mock_reset_drive(self):
        # Patch up the `reset_drive()` method to do nothing
        def mock_reset_drive(position, dry_run=False):
            print(f'mock_reset_drive(position={position}, dry_run={dry_run}')

        patch(
            'hal_402_device_mgr.params.EtherCATDriveParams.reset_drive',
            side_effect=mock_reset_drive,
        ).start()
        yield
        patch.stopall()
...
class TestHal402MgrSV660Update(TestHal402MgrIS620Update):
    drive_type = 'inovance_sv660n'
