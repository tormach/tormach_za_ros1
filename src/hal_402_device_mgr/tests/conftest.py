...
def ros_params(request, fpath):
    ...
    rosparams = getattr(request.instance, 'ros_params', {})
    if rosparams:
        # Use params from test object attrs
        return rosparams
    if getattr(request.instance, "drive_type", None):
        # Load .yaml file from current directory
        yaml_file = fpath(f"ros_params_{request.instance.drive_type}.yaml")
        with open(yaml_file) as f:
            return yaml.safe_load(f)
    return {}  # Return empty param list
...
class MockRospy(MockFixture):
    ...
    @classmethod
    def tweak_fixture(cls, mock_obj, obj):
        class NewROSInterruptException(Exception):
            pass

        obj.request_inst.ros_interrupt_exception = NewROSInterruptException
...
@pytest.fixture
def mock_master_online(request):
    """Fixture for mocking `rosgraph.is_master_online`

    Returns test instance `master_online` attribute, default `True`
    """

    def mock_method():
        online = getattr(request.instance, 'master_online', True)
        print(f"mock_master_online() = {online}")
        return online

    patch('rosgraph.is_master_online', side_effect=mock_method).start()
    yield mock_method
    patch.stopall()
