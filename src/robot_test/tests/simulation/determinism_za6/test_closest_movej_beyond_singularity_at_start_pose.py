...
def program():
    ...
    def main():
        ...
        """
        movej(j[0.0, 0.0, 0.0, 0.0, 0.09, 0.0], velocity_scale=0.3)
        # pause()
        # movej(j[0.0, 0.0, 0.0, 0.0, 0.088, 0.0], velocity_scale=0.3)
        # movej(j[0.0, 0.0, 0.0, 0.0, 0.088, 0.0], velocity_scale=0.3)
        pause()
...
def test_movej_closest_pose_2(launcher, ik_client):
    expected_joints = [0.0, 0.4665, 0.2744, -0.010, 0.1081, 0.012]
    response = ik_client.send_closest_service_request(
    ...
    assert response.arm_config == ArmConfigs.NUT
    assert response.rev_count == 0
    assert response.joint_values == pytest.approx(expected_joints, abs=0.01)


def test_movej_closest_pose_force_config(launcher, ik_client):
    expected_arm_config = ArmConfigs.FUT
    expected_joints = [0.2899, -0.0585, -0.0580, 2.1037, -1.7560, -1.6882]
