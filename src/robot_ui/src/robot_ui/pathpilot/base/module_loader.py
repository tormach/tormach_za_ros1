...
def recursively_register_types(root_path, ignore=None):
    ignore = set() if ignore is None else set(ignore)
    if root_path not in sys.path:
        sys.path.insert(0, root_path)

    for root, dirs, files in os.walk(root_path):
        if set(root.split('/')).intersection(ignore):
            continue
        ...
        for file in files:
            ...
            with open(path) as f:
                data = f.read()
            if (
                'QML_IMPORT_NAME' in data and 'QML_IMPORT_MAJOR_VERSION' in data
            ) or (file == '__init__.py' and 'def register_types()' in data):
